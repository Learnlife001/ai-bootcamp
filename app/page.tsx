...
const acceleratorProjects = [
  { title: "Sukuna Archive", type: "AI Website", description: "An immersive anime inspired experience combining storytelling, motion, visual design and AI assisted development.", href: "https://www.instagram.com/reel/Db02uQkO-gZ/", live: "https://ryomen-sukuna-archive.vercel.app", theme: "sukuna" },
  { title: "AI Brand Content", type: "UGC & Video", description: "Product storytelling for skincare, body care and fabrics, created with consistent models and connected AI generated scenes.", href: "https://www.instagram.com/reel/Db05ePCjfe4/", theme: "ugc" },
  { title: "Medicube Campaign", type: "Collaborative Content", description: "A collaborative skincare campaign presenting Medicube Zero Pore Pad 2.0 through short form visual content.", href: "https://www.instagram.com/p/Db1YR66DTCg/", theme: "medicube" },
  { title: "5 Things I Do With Claude AI", type: "AI Education", description: "A practical breakdown of how I use AI for studying, debugging, career applications, product planning and video workflows.", href: "https://www.instagram.com/reel/Db6GbKTOYio/", theme: "claude" },
];
...
function FadeIn({ children, delay = 0, className = "" }: { children: React.ReactNode; delay?: number; className?: string }) {
  return <motion.div className={className} variants={reveal} initial={false} whileInView="visible" viewport={{ once: true, amount: 0.05 }} transition={{ delay }}>{children}</motion.div>;
}
...
function ProjectCard({ project, index }: { project: (typeof engineeringProjects)[number]; index: number }) {
  ...
  return <div ref={ref} className="project-stage"><motion.article className={`project-card ${project.accent}`} style={{ scale, top: 92 + index * 18 }}>
...
export default function Home() {
  ...
  const workOverviewItems = [...engineeringProjects.map((p) => p.title), ...acceleratorProjects.map((p) => p.title), "AI Lead Automation", "POD 7"];
  return <main className="site-shell">
    ...
    <section className="work-overview-section" aria-label="Selected work"><div className="work-overview-heading"><p className="section-label">Selected work</p><p>Engineering, AI content and collaborative projects</p></div><div className="work-overview-grid">{workOverviewItems.map((item, index) => <motion.div className={`marquee-tile tile-${index % 5}`} key={item} whileHover={{ y: -6 }}><span>{String(index + 1).padStart(2, "0")}</span>{index >= engineeringProjects.length ? <Sparkles size={24} /> : null}<strong>{item}</strong></motion.div>)}</div></section>
